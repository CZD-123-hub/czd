...
@ConditionalOnProperty(name = "llm.qwen.api-key")
public class QwenProvider implements LlmProvider {
    ...
    public QwenProvider(LlmConfig llmConfig, ObjectMapper objectMapper) {
        this.config = llmConfig.getQwen();
        ...
        return webClient.post()
                ...
                .map(data -> {
                    try {
                        ...
                        log.error("Error parsing Qwen SSE data: {}", e.getMessage());
                        return "";
                    }
                })
                .filter(s -> !s.isEmpty());
    ...
    public String chat(List<ChatMessage> messages, String systemPrompt) {
        ...
        try {
            ...
            log.error("Error parsing Qwen response: {}", e.getMessage());
